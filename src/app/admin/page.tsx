    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-300">
            <div>
                ...
                <p className="text-slate-500">Monitor system wide transactions and platform revenue.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <p className="text-sm font-medium text-slate-500 mb-1">Total Volume</p>
                    <p className="text-2xl font-bold text-slate-900">${totalVolume.toFixed(2)}</p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                    <p className="text-sm font-medium text-slate-500 mb-1">Platform Revenue</p>
                    <p className="text-2xl font-bold text-emerald-600">${appRevenue.toFixed(2)}</p>
                </div>
                <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-200 bg-slate-50">
                    <h2 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">All Transactions</h2>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {transactions?.map((tx) => (
                                <tr key={tx.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="font-medium text-slate-900">{tx.users?.full_name || 'Unknown'}</div>
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        <div className="text-xs text-slate-500">{tx.recipient_number}</div>
                                    </td>
                                    <td className="px-6 py-4 text-right font-medium text-slate-900">
                                        ${Number(tx.amount_sent).toFixed(2)}
                                    </td>
                                    <td className="px-6 py-4 text-right font-medium text-emerald-600">
                                        ${Number(tx.app_fee).toFixed(2)}
                                    </td>
                                    <td className="px-6 py-4">
